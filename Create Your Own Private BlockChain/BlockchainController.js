...
class BlockchainController {
    ...
    // Endpoint that allows user to request Validation of the chain (Post Endpoint)
    requestChainValidation() {
        this.app.post("/chain/validation", async (req, res) => {
            const errorLogs = await this.blockchain.validateChain();
            if (errorLogs.length === 0) {
                return res.status(200).json("Chain is Valid.");
            } else {
                return res.status(200).send("Chain is Invalidated!" + errorLogs);
            }
