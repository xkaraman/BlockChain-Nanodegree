...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (height === null) reject(Error("Failed to get Chain Height"));

            let newBlock = block;

            if (height >= 0) {
                // Non-Genesis Block have previous block hash
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach((block) => {
                block.getBData().then(value => { if (value.owner === address) stars.push(value) })
            ...
            self.chain.forEach((block) => {
                ...
                if (block.height > 0) {
                    ...
                    if (blockhash != previousBlockHash) {
                        errorLog.push('Error at block height ${block.height}');
                    }
                }
